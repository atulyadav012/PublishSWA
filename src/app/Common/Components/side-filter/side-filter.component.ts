...
export class SideFilterComponent implements OnInit {
  ...
  GetDynamicCaseType() {
    ...
    } else if (this.CalledFor == 'Attorney' || (this.CalledFor == 'Application' &&(this.authService.currentUserValue.profileType == 'E' || this.authService.currentUserValue.profileType =='EA'))) {
      this.placeHolder = 'Select Practice Area';
      this.caption = 'Practice Area';
    } 
    // else if (this.CalledFor == 'Attorney' || (this.CalledFor == 'Application' && this.authService.currentUserValue.profileType =='EA')) {
    //   this.placeHolder = 'Select Practice Area';
    //   this.caption = 'Practice Area';
    //   //This condition was added by nirav on 23-May-2022 for Work Item # 514. On Invited Page Side filter was not shoiwng Case type or Practice area
    // }
  ...
  onDateFromChange(fieldName: string) {
    ...
    this.appearList = this.dateRangePipe.transform(this.listCopyForRateRange?.length > 0 ? this.listCopyForRateRange : this.appearListCopy, DateFrom ? DateFrom.toDateString() : "", DateTo ? DateTo.toDateString() : "", fieldName, this.CalledFor, this.authService.currentUserValue.profileType || '');
    this.FilteredList.emit({ list: this.appearList, selectedStateText: this.selectedStateText, selectedCaseTypeText: this.selectedCaseTypeText });
  }
